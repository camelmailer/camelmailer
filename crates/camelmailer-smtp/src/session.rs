...
use camelmailer_core::{
    Credential, IdempotencyRequest, MessageScope, MessageSink, QueueMessagesOutcome, QueuedMessage,
    ResolvedRoute, RouteMode, Server, Store,
};
...
// Matched lowercase against an already-normalized header map, so the
// casing a client sends never mattered. The documented spelling is
// `CamelMailer-Idempotency-Key`, frozen as shipped even though the product
// is written `Camelmailer`; the replies below name it that way.
const IDEMPOTENCY_HEADER: &str = "camelmailer-idempotency-key";
...
pub struct Session {
    ...
    data: Option<Vec<u8>>,
    /// Byte offset where the client-supplied DATA begins, after trace headers
    /// generated by Camelmailer. Those trace headers contain timestamps and
    /// must not affect the idempotency payload hash.
    client_data_offset: usize,
...
impl Session {
    ...
    fn rcpt_to(&mut self, data: &str) -> Reply {
        ...
        if is_return_path {
            ...
            match self.store.server(credential.server_id) {
                ...
                Some(server) => {
                    // The idempotency key arrives in DATA. Accept the envelope
                    // here so a completed send can replay at the quota limit.
                    self.recipients.push(Recipient {
    ...
    fn data_command(&mut self) -> Reply {
        ...
        self.client_data_offset = data.len();
        self.data = Some(data);
    ...
    fn finish_data(&mut self) -> Reply {
        ...
        let idempotency_key = match headers.get(IDEMPOTENCY_HEADER) {
            ...
            Some(values) if values.len() != 1 => {
                self.transaction_reset();
                self.state = State::Welcomed;
                return Reply::line("554 Invalid CamelMailer-Idempotency-Key");
            }
            Some(values) => {
                if self
                    .recipients
                    .iter()
                    .any(|recipient| recipient.kind != RecipientKind::Credential)
                {
                    self.transaction_reset();
                    self.state = State::Welcomed;
                    return Reply::line(
                        "554 CamelMailer-Idempotency-Key is only valid for authenticated sends",
                    );
                }
        ...
        let data = if idempotency_key.is_some() {
            strip_message_header(&data, IDEMPOTENCY_HEADER)
        } else {
            data
        };
        ...
        let idempotency = idempotency_key.map(|key| IdempotencyRequest {
            ...
            request_hash: smtp_request_hash(
                &mail_from,
                &recipients,
                &data[self.client_data_offset.min(data.len())..],
            ),
            operation: "smtp.send".into(),
        ...
        let outgoing_count = queued
            .iter()
            .filter(|m| m.scope == MessageScope::Outgoing)
            .count() as i64;
        // Read current usage for every DATA transaction, rather than caching
        // it across submissions on a persistent connection.
        let allowance = self
            .credential
            .as_ref()
            .and_then(|credential| self.store.server(credential.server_id))
            .filter(|server| server.send_limit.is_some())
            .map(|server| camelmailer_core::SendAllowance {
                limit: server.send_limit,
                used: self.store.send_usage(server.id),
            })
            .unwrap_or_else(camelmailer_core::SendAllowance::unlimited);
        let outcome = if idempotency.is_some() {
            self.sink.queue_messages(queued, idempotency, allowance)
        } else if !allowance.allows(outgoing_count) {
            QueueMessagesOutcome::LimitExceeded(allowance.rejection_message())
        } else {
            for message in queued {
                self.sink.queue_message(message);
            }
        ...
        self.state = State::Welcomed;
        match outcome {
            QueueMessagesOutcome::Stored | QueueMessagesOutcome::Replayed => Reply::line("250 OK"),
            QueueMessagesOutcome::Conflict => {
                Reply::line("554 Idempotency key reused with a different request")
            }
            QueueMessagesOutcome::LimitExceeded(message) => {
                Reply::Line(format!("550 5.7.1 {message}"))
            }
            QueueMessagesOutcome::Failed(error) => {
                tracing::error!(%error, "failed to store idempotent SMTP message");
                Reply::line("451 Temporary storage failure")
            }
        }
    }
...
/// Remove one message header, including any folded continuation lines, while
/// preserving every other byte and the message body.
fn strip_message_header(raw: &[u8], name: &str) -> Vec<u8> {
    ...
    for line in raw.split_inclusive(|byte| *byte == b'\n') {
        ...
        skipping = content
            .split(|byte| *byte == b':')
            .next()
            .is_some_and(|key| key.eq_ignore_ascii_case(name.as_bytes()));
        if !skipping {
            out.extend_from_slice(line);
        }
    }
...
fn smtp_request_hash(mail_from: &str, recipients: &[Recipient], client_data: &[u8]) -> String {
    ...
    add(&mut hasher, mail_from.as_bytes());
    hasher.update((recipients.len() as u64).to_be_bytes());
    for recipient in recipients {
        add(&mut hasher, recipient.rcpt_to.as_bytes());
    }
    add(&mut hasher, client_data);
    hasher
        .finalize()
        .iter()
...
mod helper_tests {
    ...
    #[test]
    fn strip_message_header_removes_the_header_and_its_continuations_only() {
        let raw = b"Received: by example.com\r\nCamelMailer-Idempotency-Key: invoice/123\r\n continued\r\nSubject: Test\r\n\r\nCamelMailer-Idempotency-Key: body text\r\n";
        assert_eq!(
            strip_message_header(raw, IDEMPOTENCY_HEADER),
            b"Received: by example.com\r\nSubject: Test\r\n\r\nCamelMailer-Idempotency-Key: body text\r\n"
        );
    }
}
