...
/// Bounced share of the window's messages, or "—" while there is nothing
/// to divide by.
export function bounceRate(counters: VolumeCounters): string {
  if (counters.total === 0) return "—"
  return `${((counters.bounced / counters.total) * 100).toFixed(1)}%`
}

/// Below this many messages a bounce share says more about the sample
/// than about the sender, so the bounce flag stays quiet.
...
export const FLAG_LABELS = ["New sender", "Held mail", "Check bounces", "Failing"] as const
...
export function flags(row: VolumeRow, now: number = Date.now()): Flag[] {
  ...
  if (first !== null && now - first < 24 * 60 * 60 * 1000 && row.day.total > 0) {
    found.push({
      label: "New sender",
      tone: "teal",
      reason: `started sending ${plural(row.day.total, "message")} in the last 24 hours`,
    })
  }
  if (row.month.total >= BOUNCE_FLOOR && row.month.bounced / row.month.total > BOUNCE_LIMIT) {
    found.push({
      label: "Check bounces",
      tone: "red",
      reason: `${bounceRate(row.month)} of 30-day mail bounced`,
    })
  }
  if (row.month.held > 0) {
    found.push({
